...
#ifdef DEBUG_GRAPHICS_LANE_MARKER
	drawRoadMarker(vehicle, 1, 1, 1);
#endif // DEBUG_GRAPHICS_LANE_MARKER
...
#ifdef DEBUG_GRAPHICS_LANE_MARKER
	//node box:
	GRAPHICS::DRAW_BOX(node.coord.x - 0.4, node.coord.y - 0.4, node.coord.z - 0.4, node.coord.x + 0.4, node.coord.y + 0.4, node.coord.z + 0.4, 255, 0, 0, 255);
	//link arrow: 
	GRAPHICS::DRAW_MARKER(21, link.coord.x, link.coord.y, link.coord.z, link.direction.x, link.direction.y, link.direction.z, 90.0, -90.0, 0, 2, 2, 2, 0, 255, 0, 255, false, false, 2, false, nullptr, nullptr, false);
	//lane box:
	GRAPHICS::DRAW_BOX(pointPair[0].coord.x - 0.2, pointPair[0].coord.y - 0.2, pointPair[0].coord.z - 0.2, pointPair[0].coord.x + 0.2, pointPair[0].coord.y + 0.2, pointPair[0].coord.z + 0.2, 0, 0, 255, 255);
	GRAPHICS::DRAW_BOX(pointPair[1].coord.x - 0.2, pointPair[1].coord.y - 0.2, pointPair[1].coord.z - 0.2, pointPair[1].coord.x + 0.2, pointPair[1].coord.y + 0.2, pointPair[1].coord.z + 0.2, 0, 0, 255, 255);
#endif // DEBUG_GRAPHICS_LANE_MARKER
...
void LaneRewarder::populateNodes(const char* pathsfile){
	...
	tLink *tmplinks = (tLink*)malloc(80592 * sizeof(tLink)); //Too large for the stack, need to store in the heap
	int i = 0;
	...
	printf("\nreward:finishing reading vehiclenode and vehiclelink, node=%d, link=%d", tmpnodes.size(), i);
#endif // DEBUG_LOG
	free(tmplinks);
}
...
void LaneRewarder::drawRoadMarker(Vehicle vehicle, bool drawNode, bool drawLink, bool drawPairPoints)
{
	Vector3 currentPosition;
	int nodeID, red, green, blue;
	tNode node;
	tLink link;
	char width[20];
	memset(width, 0, 20);
	...
	for (int i = 1; i <= 6; i++) {
		...
		if (node.attr.highway) { red = 255, green = 0, blue = 0; }
		if (node.attr.cantGoLeft) { red = 255, green = 128, blue = 0; }
		GRAPHICS::DRAW_BOX(node.coord.x - 0.4, node.coord.y - 0.4, node.coord.z - 0.4, node.coord.x + 0.4, node.coord.y + 0.4, node.coord.z + 0.4, red, green, blue, 255);
		...
		for (int j = 0; j < node.links.size(); j++) {
			...
			if (link.attr.shortcut) { red = 255, green = 0, blue = 0; }
			if (link.attr.narrowRoad) { red = 255, green = 128, blue = 0; }
			if (link.attr.width == -1) { red = 128, green = 0, blue = 255; }
			GRAPHICS::DRAW_MARKER(21, link.coord.x, link.coord.y, link.coord.z, link.direction.x, link.direction.y, link.direction.z, 90.0, -90.0, 0, 2, 2, 2, red, green, blue, 255, false, false, 2, false, NULL, NULL, false);
		}
	}
}
